module AnimationExample exposing (main)
    ...
    let
        ...
        defaultCylinder =
            Cylinder3d.along Axis3d.x
                { start = Length.cssPixels -1
                , end = Length.cssPixels 3
                , radius = Length.cssPixels 1
                }

        testCylinder =
            Scene3d.cylinder (Material.color Color.blue) <|
                Maybe.withDefault defaultCylinder <|
                    Cylinder3d.from
                        (Point3d.meters 0 0 0)
                        (Point3d.xyz (Length.cssPixels 0) (Length.cssPixels 10) (Length.cssPixels 2))
                        (Length.cssPixels 3)
    in
